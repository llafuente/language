...
module.exports = function(ranges) {
   ...
   */
  function test(str) {
    log("test", inspect(str), inspect(state.ctt));
  ...
  */
  ...
  function error(message, force_return) {
    if (state.look_ahead && !force_return) {
      log('\u001b[31m' + message + '\u001b[39m', JSON.stringify(state), new Error().stack);
      return null;
    }
