...
function parse(code, verbose) {
  ...
  function stack_is(str) {
    return !stack.length ? false : str === stack.join("");
  }
  ...
  mainloop: for (i = 0; i < max; ++i) {
    ...
    if (
      ...
      stack.push(code[i]);
      last_was_blank = true;
      verbose && console.log("blank");
      continue;
    }

    /* allow dash need to be fixed
    var next2 = next.substr(0, 2);
    ...
    */
    ...
    for (z = 0; z < zmax; ++z) {
      if (!stack_is("operator") && next.indexOf(operators[z]) === 0) {
        verbose && console.log("operator!");
        pop_stack();
