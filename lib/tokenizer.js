...
var exprs = require("./parser.expression.js");
var operators = []
  // ternary expr
  .concat(exprs.conditional_operators)
  // assignaments
  .concat(exprs.assignment_operators)
  // unary expr
  .concat(exprs.right_unary_operators)
  .concat(exprs.left_unary_operators)
  // binary expr
  .concat(exprs.logical_or_operators)
  .concat(exprs.logical_and_operators)
  .concat(exprs.bitwise_or_operators)
  .concat(exprs.bitwise_xor_operators)
  .concat(exprs.bitwise_and_operators)
  .concat(exprs.equality_operators)
  .concat(exprs.relational_operators)
  .concat(exprs.shift_operators)
  .concat(exprs.additive_operators)
  .concat(exprs.multiplicative_operators)
  .sort(function (a, b) { return b.length - a.length;});
var tokens = require("./parser.tokens.js");
var punctuation = tokens.punctuation;
...
function parse(code, verbose) {
  ...
  mainloop: for (i = 0; i < max; ++i) {
    ...
    /* punctuation*/
    z = 0,
    zmax = punctuation.length;

    for (z = 0; z < zmax; ++z) {
      if (next.indexOf(punctuation[z]) === 0) {
        pop_stack();
        tokens.push(punctuation[z]);
        i += punctuation[z].length - 1;
        continue mainloop;
      }
    }
    ...
    if (c === ".") {
      ...
      verbose && console.log("dot", stack);
      if (!stack.length || !stack.join("").match(/^[0-9]*$/)) {
        pop_stack();
        tokens.push(".");
      } else {
        stack.push(".");
    ...
    if (c === "-" || c === "+" || c === "@") {
      if (last_was_blank || !stack.length) {
        pop_stack();
        tokens.push(c);
      } else {
        stack.push(c);
