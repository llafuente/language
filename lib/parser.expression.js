...
function read_expression_new() {
  ...
  return error("not-implemented", true);
}
...
function read_property_access() {
  ...
  return error("expected: property access", true);
}
...
function read_index_access() {
  ...
  return error("expected: index access", true);
}
...
function read_expression_new_with_arguments() {
  ...
  if (accept("new")) {
    ...
    if (is_error(ast.callee)) {
      return error("expected member expression", true);
    }
    ast.arguments = read_arguments();
    if (is_error(ast.arguments)) {
      return error("expected member arguments", true);
    }

    return ast_end(ast);
  }
  return error("expected new expression with arguments", true);
}
...
function read_primary_expression() {
  ...
  if (is_error(lit)) {
    ...
    return error("expected primary expression", true);
  }
