...
function read_numeric_literal() {
  ...
  // test 0x hexadecimal
  if (test(/^0[x#][0-9a-fA-F]*$/)) {
    ast.value = parseInt(eat(true).substring(2), 16);
    return ast_end(ast);
  }

  // test 0c octal
  if (test(/^0[oc][0-7]*$/)) {
    ast.value = parseInt(eat(true).substring(2), 8);
    return ast_end(ast);
  }

  // test 0b binary
  if (test(/^0b[0-1]*$/)) {
    ast.value = parseInt(eat(true).substring(2), 2);
    return ast_end(ast);
  }
...
function read_call_function_block () {
  var ast = ast_new("function-declaration");

  ast.id = null;
  ast.parameters = null;
  ast.return_type = null;
  ast.body = read_function_body();
